...
read_items_from_gdxs <- function(gdx_filepaths, gdx_items, remind_names = TRUE) {
  ...
  gdx_items <- purrr::map(gdx_items, ~if(!"field" %in% names(.x)) c(.x, "field" = "l") else .x)
  ...
  possible_read_gdx <- purrr::possibly(quitte::read.gdx, otherwise = tibble(warning = "Item does not exist in gdx."))
  ...
  gdx_data <- tidyr::expand_grid(file = paste(gdx_filepaths, run_names),
    ...
    # Read in data
    dplyr::mutate(my_data = purrr::map(.data$gdxs$file, ~ possible_read_gdx(.x, .data$item, .data$field)) %>%
